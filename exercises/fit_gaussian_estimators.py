...
def test_univariate_gaussian():
    ...
    go.Figure(
        [go.Scatter(x=ms, y=errors, mode='markers+lines', name=r'$distance$')],
        layout=go.Layout(
            title=r"$\text{Absolute Error of the Estimator as Function of Number of Samples}$",
    ...
    make_subplots(rows=1, cols=1) \
        .add_traces([go.Scatter(x=X, y=pdf_1, mode='markers',
                                marker=dict(color="red"), showlegend=False)]) \
        .update_layout(
        title_text=r"Empirical PDF Function under the Q1 Fitted Model",
...
def test_multivariate_gaussian():
    ...
    fitted_4.fit(X)
    print(
        f"estimated expectation: {fitted_4.mu_}\n"
    ...
    for f1 in ms:
        ...
        for f3 in ms:
            sub_arr.append(
                fitted_5.log_likelihood(np.array([f1, 0, f3, 0]), cov, X))
        log_likelihood.append(sub_arr)

    go.Figure(go.Heatmap(x=ms, y=ms, z=log_likelihood)).update_layout(
        title="Heatmap of Log-Likelihood Evaluation"
    ...
    print(
        f"f1: {round(ms[result[0]][0], 3)}, f3: {round(ms[result[1]][0], 3)}")
